...
if [ -n "$BR_IF" ]; then
  nohup timeout "$DUR" tcpdump -i "$BR_IF" -s 0 -w "$DIR/bridge_sbi.pcap" "tcp port 7777 or udp port 8805 or tcp port 5060 or udp port 5060 or tcp port 6060 or udp port 6060" >"$DIR/bridge_tcpdump.out" 2>&1 </dev/null &
  echo $! >> "$DIR/pids"
